from django.shortcuts import redirect, render_to_response, render, get_object_or_404, get_list_or_404, reverse
...
def post_by_category(request, category_slug):
	...
	return render(request, 'myapp/post_by_category.html', context)
...
def post_by_tag(request, tag_slug):
	...
	return render(request, 'myapp/post_by_tag.html', context) 
...
def feedback(request):
	if request.method == 'POST':
		...
		if f.is_valid():
			...
			topic = f.cleaned_data['subject']
			message_body = f.cleaned_data['message']
			subject = "New Feedback from {}:{}".format(name, sender)
			message = "Subject: {}\n\nMessage: {}".format(topic, message_body)
...
def test_cookie(request):
	value = request.COOKIES.get('color') 
	if not value:
		response = HttpResponse('Cookie Set')
		response.set_cookie('color', 'blue')
		return response
	else:
		return HttpResponse('Your favorite color is {0}.'.format(value))


def track_user(request):
	response = render(request, 'myapp/track_user.html') # store the response in response variable
...
def lousy_login(request):
	...
	return render(request, 'myapp/lousy_login.html')

def lousy_secret(request):
	if not request.session.get('logged_in'):
		return redirect('lousy_login')
	return render(request, 'myapp/lousy_secret_page.html')
...
def lousy_logout(request):
	try:
		del request.session['logged_in']
			...
			return redirect('lousy_login')
	return render(request, 'myapp/lousy_logout.html') 
